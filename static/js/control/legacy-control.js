...
async function loadMembers() {
    ...
    try {
        const [members, joinCodes] = await Promise.all([
            api(`/api/campaigns/${state.selectedCampaign}/members`),
            api(`/api/campaigns/${state.selectedCampaign}/join_codes`).catch(() => []),
        ]);
        state.members = members;
        state.joinCodes = joinCodes;
        if (!state.members.length) {
            el.innerHTML = '<div class="list-item-meta">No campaign members found.</div>';
            return;
        }
        el.innerHTML = state.members.map((m) => {
            const activeCode = (state.joinCodes || []).find((c) =>
                c.principal_id === m.principal_id && !c.revoked_at
            );
            const gameHref = activeCode?.join_token
                ? `/game?principal_id=${encodeURIComponent(m.principal_id)}&join_token=${encodeURIComponent(activeCode.join_token)}`
                : `/game?principal_id=${encodeURIComponent(m.principal_id)}`;
            return `
                <div class="list-item">
                    <div class="list-item-info">
                        ...
                        <div class="list-item-meta">${activeCode ? `Join code active: ${escapeHtml(activeCode.id.substring(0, 8))}` : 'No active join code'}</div>
                    </div>
                    <div class="list-item-actions">
                        <button class="btn btn-secondary" onclick="window.control.createJoinCode('${m.principal_id}')">Create Join Code</button>
                        ${activeCode ? `<button class="btn btn-danger" onclick="window.control.revokeJoinCode('${activeCode.id}')">Revoke</button>` : ''}
...
async function createJoinCode(principalId) {
    if (!state.selectedCampaign) { showErr('Select a campaign first'); return; }
    try {
        const payload = { principal_id: principalId, label: 'Control Plane join code' };
        if (state.selectedSession) payload.session_id = state.selectedSession;
        const result = await api(`/api/campaigns/${state.selectedCampaign}/join_codes`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(payload),
        });
        const code = result.join_code;
        if (code?.join_url) {
            await navigator.clipboard?.writeText(`${window.location.origin}${code.join_url}`).catch(() => {});
        }
        await loadMembers();
        showErr(code?.join_url ? `Join code created. Link copied: ${code.join_url}` : 'Join code created.');
    } catch (e) {
        showErr(e.message);
    }
}
...
async function revokeJoinCode(codeId) {
    if (!state.selectedCampaign) { showErr('Select a campaign first'); return; }
    try {
        await api(`/api/campaigns/${state.selectedCampaign}/join_codes/${codeId}`, { method: 'DELETE' });
        await loadMembers();
        showErr('Join code revoked.');
    } catch (e) {
        showErr(e.message);
    }
}
...
async function loadAi() {
    ...
    try {
        ...
        const provider = (c.llm_provider || 'mock').toLowerCase();
        const embedStatus = $('embedStatus');
        if (embedStatus) {
            if ((provider === 'lmstudio' || provider === 'ollama') && !c.embedding_model) {
                embedStatus.textContent = 'Embedding model required for RAG. Use an embedding-capable local model or leave RAG disabled.';
            } else if (c.embedding_model) {
                embedStatus.textContent = `RAG embeddings: ${c.embedding_model}`;
            } else {
...
window.control = {
    ...
    async supersedePatch(pid, patchJSON, visibility, summary) {
        const replacementSummary = prompt('Replacement summary:', summary || '');
        if (!replacementSummary) return;
        let replacementPatch = patchJSON || {};
        const raw = prompt('Replacement patch JSON:', JSON.stringify(replacementPatch, null, 2));
        if (raw) {
            try {
                replacementPatch = JSON.parse(raw);
            } catch (e) {
                showErr('Replacement patch JSON is invalid: ' + e.message);
                return;
            }
        }
        try {
            await api(`/api/patches/${pid}/supersede`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    notes: 'Superseded from Control Plane',
                }),
            });
            await window._reloadIntel?.();
            showErr('Patch superseded with a new pending replacement.');
        } catch (e) {
            showErr(e.message);
        }
    },
...
window.addEventListener('DOMContentLoaded', async () => {
    ...
    $('listModels').onclick = async () => {
        try {
            ...
            const embedding = ids.find((id) => /embed|embedding|nomic|bge|e5/i.test(id));
            if (embedding && !$('embedModel').value) {
                $('embedModel').value = embedding;
                if ($('embedStatus')) $('embedStatus').textContent = `Detected embedding model: ${embedding}`;
            } else if (($('provider').value === 'lmstudio' || $('provider').value === 'ollama') && !embedding && $('embedStatus')) {
                $('embedStatus').textContent = 'No embedding-capable model detected from local provider.';
            }
        } catch (e) {
            showErr(e.message);
        }
        ...
        try {
            ...
            listEl.innerHTML = patches.map(p => {
                ...
                const evidence = (p.evidence || []).map(e => {
                    const seq = e.seq_id || e.sequence || e.source_id || e.ledger_seq_id;
                    ...
                    const quote = e.quote ? `"${escapeHtml(e.quote)}"` : 'ledger evidence';
                    return `<div class="patch-evidence">${quote}${seq ? ` | ledger seq/id: ${escapeHtml(String(seq))}` : ''}</div>`;
                }).join('');
                ...
                <div class="patch-card kind-${kindClass}" data-pid="${p.id}">
                    <div class="patch-head">
                        <label style="display:flex;gap:6px;align-items:center;">
                            <input type="checkbox" class="intel-patch-select" value="${p.id}">
                    ...
                    <div class="patch-actions">
                        ...
                        <button class="btn btn-secondary btn-sm" onclick="window.control.supersedePatch('${p.id}', ${JSON.stringify(p.patch || {}).replace(/"/g, '&quot;')}, '${p.visibility}', '${escapeHtml(p.summary).replace(/'/g, '&#39;')}')">Supersede</button>
    ...
    const selectedPatchIds = () => Array.from(document.querySelectorAll('.intel-patch-select:checked')).map(el => el.value);
    if ($('btnIntelBatchApprove')) $('btnIntelBatchApprove').onclick = async () => {
        const patchIds = selectedPatchIds();
        if (!patchIds.length) { $('intelOut').textContent = 'Select at least one patch.'; return; }
        try {
            const result = await api('/api/patches/batch', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'approve', patch_ids: patchIds }),
            });
            $('intelOut').textContent = `Batch approved ${result.results?.length || 0} patch(es). Regenerating recaps is recommended after edits.`;
            await loadPendingPatches();
            await loadContinuity();
            await loadRecap('dm');
        } catch (e) { $('intelOut').textContent = 'Error: ' + e.message; }
    };
    if ($('btnIntelBatchReject')) $('btnIntelBatchReject').onclick = async () => {
        const patchIds = selectedPatchIds();
        if (!patchIds.length) { $('intelOut').textContent = 'Select at least one patch.'; return; }
        const notes = prompt('Reason for rejecting selected patches (optional):') || '';
        try {
            const result = await api('/api/patches/batch', {
                method: 'POST', headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'reject', patch_ids: patchIds, notes }),
            });
            $('intelOut').textContent = `Batch rejected ${result.results?.length || 0} patch(es).`;
            await loadPendingPatches();
            await loadContinuity();
        } catch (e) { $('intelOut').textContent = 'Error: ' + e.message; }
    };
