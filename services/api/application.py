...
def help_page(page="index"):
    ...
    if not wiki_file.exists() or not str(wiki_file.resolve()).startswith(
...
def readyz():
    ...
    return jsonify(
        {"ready": ok, "status": "ready" if ok else "not_ready", "checks": checks}
    ), code
...
def _get_ai_config(campaign_id: str):
    ...
    if row:
        ...
        data["embedding_model"] = select_embedding_model(
            provider, data.get("embedding_model")
        )
        return data
    return {
        ...
        "embedding_model": select_embedding_model("mock"),
        "settings": {},
    }
...
def _openai_client_for(provider: str, base_url: str | None):
    ...
    api_key = (
        os.environ.get(f"{(provider or '').upper()}_API_KEY")
...
def _extract_text_chunks(storage_path: str):
    ...
    if suffix == ".pdf":
        ...
        pages.append(
            (1, Path(storage_path).read_text(encoding="utf-8", errors="ignore"))
        )
    ...
    for page_num, text in pages:
        ...
        while start < len(clean):
            ...
            chunk_id = hashlib.sha1(
                f"{page_num}:{idx}:{chunk_text}".encode()
            ).hexdigest()
...
def _active_collection_for(
    campaign_id: str, provider: str, embedding_model: str
) -> tuple[str, str]:
    ...
    if profile is None:
        raise ValueError("embedding_profile_unavailable")
...
def _search_qdrant(
    campaign_id: str, query_vector: list[float], top_k: int
) -> list[dict]:
    collection = _qdrant_collection_name(campaign_id)
...
def _create_entity_record(campaign_id: str, data: dict):
    ...
    if controlled_by in {"AI", "AI_NPC"} and not controller_principal_id:
        ...
        controller_principal_id = str(
            ensure_ai_controller_principal(uuid.UUID(str(campaign_id)))
        )
...
def _process_rag_document(doc_id: str):
    ...
    try:
        ...
        embedding_result = (
            llm.embed_texts([c["text"] for c in chunks])
            if chunks
            else {"vectors": [], "dimensions": 0}
        )
        ...
        execute_query(
            "DELETE FROM state.rag_chunks WHERE doc_id=%s", (doc_id,), fetch=False
        )
        if dims > 0:
            try:
                _qdrant_request(
            ...
            except Exception:  # defensive boundary
                logging.exception(
                    "Failed deleting prior qdrant points for doc_id=%s", doc_id
                )
...
@api_bp.route("/api/campaigns/<campaign_id>")
...
@api_bp.route("/api/campaigns/<campaign_id>/entities")
...
@api_bp.route("/api/campaigns/<campaign_id>/encounters", methods=["GET", "POST"])
...
@api_bp.route("/api/campaigns/<campaign_id>/members", methods=["GET", "POST"])
def api_campaign_members(campaign_id):
    ...
    if request.method == "POST":
        ...
        auth_subject = (
            data.get("auth_subject")
            or f"local:{campaign_id}:{display_name.lower().replace(' ', '-')}"
        )
...
@api_bp.route("/api/campaigns/<campaign_id>/join_codes", methods=["GET", "POST"])
def api_campaign_join_codes(campaign_id):
    from shared.db.connection import execute_one
    ...
    if strict_join_tokens_required() or _principal_id_from_request():
        principal, error = _require_campaign_gm(campaign_id, require_join_token=True)
        if error:
            return error
    ...
    if request.method == "GET":
        return jsonify([_serialize(row) for row in list_join_codes(campaign_id)])
    ...
    try:
        uuid.UUID(str(principal_id))
        session_id = data.get("session_id")
        if session_id:
            uuid.UUID(str(session_id))
    except (TypeError, ValueError):
        return jsonify({"error": "invalid principal_id or session_id"}), 400

    member = execute_one(
        """
        SELECT cm.principal_id
        FROM state.campaign_members cm
        WHERE cm.campaign_id = %s AND cm.principal_id = %s
        """,
        (campaign_id, str(principal_id)),
    )
    if not member:
    ...
    return jsonify({"ok": True, "join_code": _serialize(code)})


@api_bp.route("/api/campaigns/<campaign_id>/join_codes/<code_id>", methods=["DELETE"])
def api_campaign_join_code_revoke(campaign_id, code_id):
    from services.api.local_identity import (
        revoke_join_code,
        strict_join_tokens_required,
    )
    ...
    row = revoke_join_code(campaign_id=campaign_id, code_id=code_id)
    if not row:
        return jsonify({"error": "join code not found"}), 404
    return jsonify({"ok": True, "join_code": _serialize(row)})


# Shared route/auth helpers live in services.api.auth and services.api.responses.


@api_bp.route("/api/campaigns/<campaign_id>/session")
...
@api_bp.route("/api/sessions/<session_id>/join", methods=["POST"])
def api_join_session(session_id):
    ...
    require_token = strict_join_tokens_required()
    if supplied_token or require_token:
        ...
        if not validation.ok:
            return jsonify({"error": validation.reason or "Invalid join token"}), 403
...
@api_bp.route("/api/sessions/<session_id>/player_state")
def api_player_state(session_id):
    from services.player_state.snapshot import (
        PlayerStateError,
        build_player_state_snapshot,
    )
    from services.api.local_identity import strict_join_tokens_required
    ...
    if strict_join_tokens_required():
        ...
        if error:
            ...
            try:
                ...
                body = {}
            return jsonify(
                {"ok": False, "error": body.get("error", "unauthorized")}
            ), status
...
@api_bp.route("/api/campaigns/<campaign_id>/mode", methods=["GET", "POST"])
...
@api_bp.route("/api/campaigns/<campaign_id>/maps")
...
@api_bp.route("/api/maps/<map_id>/children")
...
@api_bp.route("/api/maps/<map_id>/decorations")
...
@api_bp.route("/api/maps/<map_id>/pois")
...
@api_bp.route("/api/maps/<map_id>/pois", methods=["POST"])
...
@api_bp.route("/api/pois/<poi_id>", methods=["PUT"])
...
@api_bp.route("/api/pois/<poi_id>", methods=["DELETE"])
...
@api_bp.route("/api/pois/<poi_id>/image", methods=["POST"])
...
@api_bp.route("/api/saves/game/export", methods=["POST"])
...
@api_bp.route("/api/saves/game/import", methods=["POST"])
...
@api_bp.route("/api/saves/program/export", methods=["POST"])
...
@api_bp.route("/api/saves/program/import", methods=["POST"])
...
@api_bp.route("/api/global_settings", methods=["GET"])
...
@api_bp.route("/api/global_settings/<key>", methods=["PUT"])
def api_global_settings_put(key):
    ...
    return jsonify(
        {"key": row["key"], "saved": True, "updated_at": _serialize(row)["updated_at"]}
    )
...
@api_bp.route("/api/sessions/<session_id>/intel/extract", methods=["POST"])
def api_intel_extract(session_id):
    ...
    if not cid:
        ...
        row = execute_one(
            "SELECT campaign_id FROM state.sessions WHERE id=%s", (session_id,)
        )
        if not row:
            return jsonify({"error": "session not found"}), 404
...
@api_bp.route("/api/campaigns/<campaign_id>/patches")
...
@api_bp.route("/api/patches/<patch_id>/approve", methods=["POST"])
...
@api_bp.route("/api/patches/<patch_id>/reject", methods=["POST"])
...
@api_bp.route("/api/patches/batch", methods=["POST"])
def api_patches_batch():
    from shared.db.connection import execute_one
    from services.session_intel.patcher import apply_patch, PatchError

    data = request.get_json(silent=True) or {}
    patch_ids = data.get("patch_ids") or []
    ...
    if not isinstance(patch_ids, list) or not patch_ids:
        return jsonify({"error": "patch_ids must be a non-empty list"}), 400
    ...
    for pid in patch_ids:
        if action == "reject":
            row = execute_one(
                "UPDATE state.proposed_story_patches SET status = 'REJECTED', "
                "reviewed_at = now(), reviewed_by = %s, notes = %s "
                "WHERE id = %s AND status IN ('PENDING','EDITED') RETURNING id, status",
                (reviewed_by, notes, pid),
            )
            results.append(
                _serialize(row) if row else {"id": pid, "error": "not found"}
            )
            continue
        execute_one(
            "UPDATE state.proposed_story_patches SET status = 'APPROVED', "
...
@api_bp.route("/api/patches/<patch_id>/edit", methods=["POST"])
...
@api_bp.route("/api/patches/<patch_id>/supersede", methods=["POST"])
def api_patch_supersede(patch_id):
    from shared.db.connection import execute_one

    data = request.get_json(silent=True) or {}
    replacement = execute_one(
        """
        WITH original AS (
            UPDATE state.proposed_story_patches
            SET status = 'SUPERSEDED',
                reviewed_at = now(),
                reviewed_by = %s,
                notes = COALESCE(%s, notes)
            WHERE id = %s AND status IN ('APPROVED','APPLIED','EDITED','PENDING')
            RETURNING *
        )
        INSERT INTO state.proposed_story_patches (
            campaign_id, session_id, source_kind, source_ref, event_type,
            summary, entities, patch, evidence, confidence, visibility, status, notes
        )
        SELECT campaign_id, session_id, source_kind,
               source_ref || jsonb_build_object('supersedes_patch_id', id::text),
               event_type,
               COALESCE(%s, summary),
               COALESCE(%s::jsonb, entities),
               COALESCE(%s::jsonb, patch),
               evidence,
               confidence,
        ...
        (
            ...
            patch_id,
            data.get("summary"),
            json.dumps(data["entities"]) if "entities" in data else None,
            json.dumps(data["patch"]) if "patch" in data else None,
            data.get("visibility"),
        ),
    )
    if not replacement:
        return jsonify({"error": "Patch not found or cannot be superseded"}), 404
    return jsonify(_serialize(replacement))


@api_bp.route("/api/sessions/<session_id>/recap")
...
@api_bp.route("/api/sessions/<session_id>/dm_packet", methods=["POST"])
...
@api_bp.route("/api/campaigns/<campaign_id>/open_threads")
    ...
    return jsonify(
        [
            ...
            for r in open_threads(
                ...
                principal_id=None
                if visibility == "dm"
                else str(principal.principal_id),
            )
        ]
    )


@api_bp.route("/api/campaigns/<campaign_id>/unresolved_promises")
...
@api_bp.route("/api/campaigns/<campaign_id>/npc_memory/<npc_id>")
...
@api_bp.route("/api/_debug/save_canvas", methods=["POST"])
...
@api_bp.route("/api/entities/<entity_id>/transition_map", methods=["POST"])
...
@api_bp.route("/api/events/audience", methods=["POST"])
...
@api_bp.route("/api/decorations/<deco_id>/generate_image", methods=["POST"])
...
@api_bp.route("/api/campaigns/<campaign_id>/test_image_gen", methods=["POST"])
...
@api_bp.route("/api/pois/<poi_id>/generate_image", methods=["POST"])
...
@api_bp.route("/api/entities/<entity_id>/discovered_pois")
def api_discovered_pois(entity_id):
    ...
    visible = [
        p
        for p in discovered_pois(dict(entity), [dict(p) for p in pois])
        if not p.get("is_hidden")
    ]
    return jsonify([_serialize(p) for p in visible])


@api_bp.route("/api/propose", methods=["POST"])
def api_propose():
    data = request.get_json(silent=True) or {}
    try:
...
@api_bp.route("/api/dice/roll", methods=["POST"])
def api_roll_dice():
...
@api_bp.route("/api/encounters/<encounter_id>/advance", methods=["POST"])
def api_advance_turn(encounter_id):
    ...
    principal, error = _require_campaign_gm(
        str(encounter["campaign_id"]), require_join_token=True
    )
    if error:
        return error
...
@api_bp.route("/api/encounters/<encounter_id>/slots")
...
@api_bp.route("/api/encounters/<encounter_id>/auto_advance", methods=["POST"])
def api_auto_advance(encounter_id):
    ...
    encounter = execute_one(
        "SELECT * FROM state.encounters WHERE id = %s", (encounter_id,)
    )
    if not encounter:
        return jsonify({"error": "Encounter not found"}), 404
    ...
    session_id = uuid.UUID(
        str(encounter.get("session_id", "66666666-6666-6666-6666-666666666661"))
    )
...
@api_bp.route("/api/narrate", methods=["POST"])
def api_narrate():
    data = request.get_json(silent=True) or {}
    try:
...
@api_bp.route("/api/content/check", methods=["POST"])
...
@api_bp.route("/api/export/<session_id>")
...
@api_bp.route("/api/campaigns/<campaign_id>", methods=["PUT"])
...
@api_bp.route("/api/campaigns/<campaign_id>", methods=["DELETE"])
...
def _cascade_delete_campaign(campaign_id: str) -> bool:
    ...
    return hard_delete_campaign(campaign_id)


@api_bp.route("/api/campaigns/<campaign_id>/purge", methods=["POST"])
...
@api_bp.route("/api/campaigns/<campaign_id>/sessions")
...
@api_bp.route("/api/campaigns/<campaign_id>/sessions", methods=["POST"])
...
@api_bp.route("/api/entities/<entity_id>", methods=["PUT"])
def api_entity_update(entity_id):
    ...
    data = request.get_json(silent=True) or {}
    if data.get("controlled_by") in {"AI", "AI_NPC"} and not data.get(
        "controller_principal_id"
    ):
        entity = execute_one(
        ...
        if entity:
            from services.orchestrator.npc_autonomy import (
                ensure_ai_controller_principal,
            )
    ...
    for key in (
        ...
        if key in data:
            ...
            val = (
                json.dumps(data[key])
                if key in {"public_sheet", "secret_sheet"}
                else data[key]
            )
            params.append(val)
    if not fields:
...
@api_bp.route("/api/entities/<entity_id>/control", methods=["POST"])
def api_entity_control(entity_id):
    ...
    if data.get("controlled_by") not in {"HUMAN", "PLAYER", "GM", "AI", "AI_NPC"}:
        return jsonify(
            {"error": "controlled_by must be HUMAN, PLAYER, GM, AI, or AI_NPC"}
        ), 400
...
@api_bp.route("/api/entities/<entity_id>", methods=["DELETE"])
...
@api_bp.route("/api/entities/<entity_id>/restore", methods=["POST"])
...
@api_bp.route("/api/entities/<entity_id>/image", methods=["POST"])
...
@api_bp.route("/api/entities/<entity_id>/image", methods=["DELETE"])
...
@api_bp.route("/api/sessions/<session_id>/characters")
...
@api_bp.route("/api/sessions/<session_id>/characters", methods=["POST"])
...
@api_bp.route("/api/sessions/<session_id>/characters/<entity_id>", methods=["PUT"])
...
@api_bp.route(
    "/api/sessions/<session_id>/characters/<entity_id>/revive", methods=["POST"]
)
def api_session_character_revive(session_id, entity_id):
    ...
    data = request.get_json(silent=True) or {}
    revive_method = data.get(
        "method", "STORY"
    )  # SPELL, DIVINE_INTERVENTION, STORY, etc.
...
@api_bp.route("/api/sessions/<session_id>/characters/<entity_id>", methods=["DELETE"])
...
@api_bp.route("/api/campaigns/<campaign_id>/available_characters")
...
@api_bp.route("/api/campaigns/<campaign_id>/characters/generate", methods=["POST"])
...
- Note: ac and speed must be at the TOP LEVEL of the JSON, not inside public_sheet"""
    ...
    if populate_form:
        return jsonify(
            {
                "character": generated,
                "message": "Character generated. Review and create.",
            }
        ), 200
...
@api_bp.route("/api/campaigns/<campaign_id>/ai_config")
def api_ai_config_get(campaign_id):
    ...
    if "api_keys" in settings and isinstance(settings["api_keys"], dict):
        settings["api_keys"] = redact_dict_values(
            settings["api_keys"], sensitive_keys=None
        )
    return jsonify(cfg)


@api_bp.route("/api/campaigns/<campaign_id>/ai_config", methods=["PUT"])
def api_ai_config_put(campaign_id):
    from shared.db.connection import execute_one
    from services.llm.adapter import (
    ...
    if provider not in SUPPORTED_PROVIDERS:
        return jsonify(
            {"error": f"invalid provider — supported: {sorted(SUPPORTED_PROVIDERS)}"}
        ), 400
    ...
    embedding_model = select_embedding_model(provider, data.get("embedding_model"))
    ...
    row = execute_one(
        ...
        (
            ...
            resolve_provider_base_url(
                provider, data.get("embedding_base_url") or data.get("llm_base_url")
            ),
...
@api_bp.route("/api/ai/test_provider", methods=["POST"])
...
@api_bp.route("/api/ai/detect_local")
def api_ai_detect_local():
    ...
    base_url = (
        request.args.get("base_url") or _LOCAL_PROVIDER_DEFAULTS.get(provider) or ""
    ).strip()
    ...
    try:
        ...
        models = (
            [{"id": m.get("id")} for m in raw if isinstance(m, dict) and m.get("id")]
            if isinstance(raw, list)
            else []
        )
        from services.llm.adapter import select_embedding_model

        suggested_embedding_model = select_embedding_model(
            provider,
        ...
        return jsonify(
            {
                ...
                "models": models,
                "embedding_model": suggested_embedding_model,
                "embedding_ready": suggested_embedding_model is not None,
                "embedding_warning": None
                if suggested_embedding_model
                else "No embedding-capable model detected.",
            }
        )
...
@api_bp.route("/api/campaigns/<campaign_id>/rag/upload", methods=["POST"])
...
@api_bp.route("/api/campaigns/<campaign_id>/rag/documents")
...
@api_bp.route("/api/rag/documents/<doc_id>/enable", methods=["POST"])
def api_rag_enable(doc_id):
    ...
    return jsonify(
        _serialize(
            execute_one(
                "UPDATE state.rag_documents SET enabled=true, updated_at=now() WHERE id=%s RETURNING *",
                (doc_id,),
            )
        )
    )
...
@api_bp.route("/api/rag/documents/<doc_id>/disable", methods=["POST"])
def api_rag_disable(doc_id):
    ...
    return jsonify(
        _serialize(
            execute_one(
                "UPDATE state.rag_documents SET enabled=false, updated_at=now() WHERE id=%s RETURNING *",
                (doc_id,),
            )
        )
    )
...
@api_bp.route("/api/rag/documents/<doc_id>/reindex", methods=["POST"])
def api_rag_reindex(doc_id):
    from shared.db.connection import execute_one

    doc = execute_one(
        "SELECT campaign_id FROM state.rag_documents WHERE id=%s", (doc_id,)
    )
    if not doc:
        return jsonify({"error": "Not found"}), 404
    execute_one(
...
@api_bp.route("/api/campaigns/<campaign_id>/rag/query", methods=["POST"])
...
def handle_join_campaign(data):
    ...
    try:
        meta = validate_socket_join(
            ...
            principal_id=data.get("principal_id"),
            session_id=data.get("session_id"),
            join_token=data.get("join_token") or data.get("invite_token"),
        )
...
@api_bp.route("/api/sessions/<session_id>/story_state")
...
@api_bp.route("/api/sessions/<session_id>/story_state", methods=["PUT"])
def api_update_story_state(session_id):
    ...
    with transaction() as conn:
        ...
        cur.execute(
            "SELECT * FROM state.story_state WHERE session_id = %s", (session_id,)
        )
...
@api_bp.route("/api/sessions/<session_id>/story_state/history")
...
@api_bp.route("/api/campaigns/<campaign_id>/session_archives")
...
@api_bp.route("/api/session_archives/<archive_id>")
...
@api_bp.route("/api/session_archives/<archive_id>", methods=["DELETE"])
...
@api_bp.route("/api/sessions/<session_id>/chat_history")
...
@api_bp.route("/api/sessions/<session_id>/resume_data")
    ...
    return jsonify(
        {
            ...
            "story_state": (
                ...
                else _redact_story_state(story_state)
                if story_state
                else None
            ),
...
@api_bp.route("/api/sessions/<session_id>/archive", methods=["POST"])
