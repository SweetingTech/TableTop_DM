...
class VisibilityResolver:
    ...
    def resolve_visible_to(self, action_type: str, campaign_id: uuid.UUID,
                           sender_principal_id: uuid.UUID,
                           encounter_id: Optional[uuid.UUID] = None,
                           target_entity_id: Optional[uuid.UUID] = None) -> list[uuid.UUID]:
    ...
    def filter_event_for_principal(self, event: dict, principal_id: uuid.UUID) -> Optional[dict]:
        visible_to = event.get("visible_to", [])
        if str(principal_id) in [str(v) for v in visible_to]:
            return event
        return None

    def filter_events_for_principal(self, events: list[dict], principal_id: uuid.UUID) -> list[dict]:
