...
Provide dramatic, atmospheric narration of what happened. Do NOT change any mechanical results."""
    ...
    def create_narration_event(self, narration: str, campaign_id: uuid.UUID,
                               session_id: uuid.UUID, dm_principal_id: uuid.UUID,
                               visible_to: list[uuid.UUID],
                               encounter_id: Optional[uuid.UUID] = None,
                               parent_event_id: Optional[uuid.UUID] = None) -> EventEnvelope:
...
Respond in JSON with "dialogue" (what the NPC says) and "action" (brief physical action, optional)."""
    ...
    def generate_dialogue(self, npc_name: str, npc_sheet: dict,
                          emotional_state: str, context: str,
                          speaker_message: str = "") -> dict:
...
"""
    ...
    def create_dialogue_event(self, dialogue: dict, npc_name: str,
                               campaign_id: uuid.UUID, session_id: uuid.UUID,
                               npc_principal_id: uuid.UUID, npc_entity_id: uuid.UUID,
                               visible_to: list[uuid.UUID],
                               encounter_id: Optional[uuid.UUID] = None) -> EventEnvelope:
        return EventEnvelope(
