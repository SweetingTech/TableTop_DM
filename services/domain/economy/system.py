...
class EconomySystem:
    ...
    def update_location_metrics(self, location_entity_id: uuid.UUID,
        ...
        try:
            ...
            """, (
                ...
                stability_delta, scarcity_delta, prosperity_delta,
            ))
    ...
    def compute_item_price(self, shop_id: uuid.UUID, item_entity_id: uuid.UUID,
                           campaign_id: uuid.UUID) -> dict:
        shop = execute_one(
            "SELECT * FROM state.shops WHERE id = %s", (str(shop_id),)
        )
        if not shop:
        ...
        final_price = base_price * shop_multiplier * scarcity * stability_mult * extra_mult
...
class PropertySystem:
    ...
    def buy_property(self, property_entity_id: uuid.UUID, buyer_entity_id: uuid.UUID,
                     upkeep_cost: float, conn=None) -> dict:
        own_conn = conn is None
        if own_conn:
            conn = get_connection()
