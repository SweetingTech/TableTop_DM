...
from shared.schemas.enums import TensionLevel, TENSION_LADDER, GameMode
...
class SocialSystem:
    ...
    def get_tension(self, campaign_id: uuid.UUID, entity_a: uuid.UUID,
                    entity_b: uuid.UUID) -> TensionLevel:
        row = execute_one("""
    ...
    def escalate_tension(self, campaign_id: uuid.UUID,
                         entity_a: uuid.UUID, entity_b: uuid.UUID,
                         current: TensionLevel) -> dict:
    ...
    def deescalate_tension(self, campaign_id: uuid.UUID,
                           entity_a: uuid.UUID, entity_b: uuid.UUID,
                           current: TensionLevel) -> dict:
        idx = TENSION_LADDER.index(current)
        if idx <= 0:
    ...
    def resolve_social_check(self, entity_id: uuid.UUID, skill: str,
                             modifier: int, dc: int) -> ToolCallPayload:
...
class AmbushPipeline:
    ...
    def resolve_stealth(self, ambusher_id: uuid.UUID, stealth_mod: int,
    ...
    def apply_surprise(self, ambush_result: dict, encounter_id: uuid.UUID,
