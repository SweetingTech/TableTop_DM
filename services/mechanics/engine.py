...
from shared.schemas.events import StateDelta, RollResult, ToolCallPayload
from shared.schemas.enums import ConditionType
from services.mechanics.dice import roll_dice, roll_d20


class MechanicsEngine:

    def update_hp(self, entity_id: uuid.UUID, delta: int, current_hp: int,
                  max_hp: int, source: str = "unknown") -> ToolCallPayload:
    ...
    def apply_condition(self, entity_id: uuid.UUID, condition: str,
                        duration_rounds: int = -1, source: str = "unknown",
                        campaign_id: uuid.UUID = None,
                        encounter_id: uuid.UUID = None) -> ToolCallPayload:
        deltas = [StateDelta(
        ...
        return ToolCallPayload(
            ...
            arguments={"entity_id": str(entity_id), "condition": condition,
                        "duration_rounds": duration_rounds, "source": source},
    ...
    def modify_inventory(self, entity_id: uuid.UUID, item_id: uuid.UUID,
                         quantity_delta: int) -> ToolCallPayload:
        ...
        return ToolCallPayload(
            ...
            arguments={"entity_id": str(entity_id), "item_id": str(item_id),
                        "quantity_delta": quantity_delta},
            result={"entity_id": str(entity_id), "item_id": str(item_id),
                     "quantity_delta": quantity_delta},
            deltas=deltas,
        )

    def resolve_attack(self, attacker_id: uuid.UUID, target_id: uuid.UUID,
                       attack_modifier: int, target_ac: int, damage_dice: str,
                       damage_modifier: int = 0, weapon: str = "unarmed",
                       attacker_hp: int = 0, attacker_max_hp: int = 0,
                       target_hp: int = 0, target_max_hp: int = 0) -> ToolCallPayload:
        ...
        return ToolCallPayload(
            ...
            arguments={"attacker_id": str(attacker_id), "target_id": str(target_id),
                        "weapon": weapon, "damage_dice": damage_dice},
            result=result,
            deltas=deltas,
            rolls=rolls,
        )

    def resolve_save(self, entity_id: uuid.UUID, save_modifier: int,
                     dc: int, save_type: str = "DEX",
                     effect_on_fail: Optional[str] = None,
                     damage_dice: Optional[str] = None,
                     half_on_save: bool = False,
                     entity_hp: int = 0, entity_max_hp: int = 0) -> ToolCallPayload:
