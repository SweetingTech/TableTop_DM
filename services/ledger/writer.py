...
class LedgerWriter:
    ...
    def append_event(self, event: EventEnvelope, conn=None) -> dict:
        ...
        try:
            ...
            """, (
                str(event.event_id), event.event_version, event.contract_version,
                event.type.value, str(event.campaign_id), str(event.session_id),
    ...
    def read_events(self, session_id: uuid.UUID, principal_id: uuid.UUID,
                    from_seq: int = 0, limit: int = 100) -> list[dict]:
        conn = get_connection()
        try:
            cur = conn.cursor()
    ...
    def write_summary(self, campaign_id: uuid.UUID, session_id: uuid.UUID,
                      scope: str, principal_id: Optional[uuid.UUID],
                      from_seq: int, to_seq: int, summary_text: str,
                      visible_to: list[uuid.UUID], conn=None) -> dict:
        own_conn = conn is None
        if own_conn:
            conn = get_connection()
        ...
        try:
            ...
            cur.execute("""
                ...
                RETURNING id
            """, (
                str(summary_id), str(campaign_id), str(session_id), scope,
                str(principal_id) if principal_id else None,
                from_seq, to_seq, summary_text,
