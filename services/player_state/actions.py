...
def _action(
    ...
    *,
    ...
    return LegalAction(
        action=action, enabled=enabled, reason=reason, requires_target=requires_target
    )
...
def legal_actions_for(
    *,
    ...
    if not controlled_entities:
        return [
            ...
            _action(
                "talk",
                False,
                "No controlled active character in this session.",
            ...
            _action(
                "end_turn", False, "No controlled active character in this session."
            ),
        ]
    ...
    visible_targets = [
        ...
        if str(e.get("id")) not in controlled_ids
        and e.get("entity_type") in {"NPC", "PC"}
    ]
    ...
    actions = [
        ...
        _action(
            ...
            None if visible_targets else "No visible target.",
            requires_target=True,
        ),
    ]
    ...
    if mode == "COMBAT":
        actions.append(
            _action(
                ...
                None
                if is_my_turn and visible_targets
                else (
                    "Not currently this entity's turn."
                    if not is_my_turn
                    else "No visible hostile target in range."
                ),
                requires_target=True,
            )
        )
