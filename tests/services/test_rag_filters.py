...
def test_public_chunk_reaches_everywhere():
    ...
    for purpose in (
        ...
        assert (
            len(filter_chunk_visibility(chunks, purpose)) == 1
        ), f"failed for {purpose}"
...
def test_npc_dialogue_respects_known_lore_tags():
    ...
    chunks = [
        _chunk(
            visibility="party",
            tags=["chapel"],
            text="The chapel was sealed by the Order.",
        ),
        _chunk(
            visibility="party",
            tags=["unrelated-faction"],
            text="The Guild charges tolls in the south.",
        ),
    ]
