...
def test_sources_for_recap_includes_visible_rag_and_ledger_sources(monkeypatch):
    ...
    patches = [{
        ...
        "patch": {},
        "evidence": [
            {"source_kind": "ledger_event", "source_id": "42", "quote": "We ring the bell."},
            {
    ...
    monkeypatch.setattr(recap, "_patches_for", lambda session_id, visibility_filter: patches)

    sources = recap.sources_for_recap(uuid.uuid4(), "party")

    assert {"kind": "ledger_event", "id": "42", "quote": "We ring the bell.", "timestamp_start": None, "timestamp_end": None} in sources
...
def test_sources_for_party_recap_omits_dm_only_rag_sources(monkeypatch):
    ...
    assert recap.sources_for_recap(uuid.uuid4(), "dm")[0]["filename"] == "bell_dm-only.md"
