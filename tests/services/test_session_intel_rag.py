...
def test_rag_supported_llm_event_is_dm_reviewed_and_dm_only(monkeypatch):
        ...
        def generate_structured(self, **kwargs):
            ...
            return {
                "events": [{
                    ...
                    "evidence": [
                        {"source_kind": "ledger_event", "source_id": "1", "quote": "We ring the Black Bell."},
    ...
    monkeypatch.setattr(extractor, "retrieve_session_intel_context", lambda **kwargs: _rag_context())
...
def test_rag_failure_does_not_block_llm_extraction(monkeypatch):
        ...
        def generate_structured(self, **kwargs):
            return {
                "events": [{
                    ...
                    "evidence": [{"source_kind": "ledger_event", "source_id": "1", "quote": "I promise."}],
                    "confidence": 0.8,
                    "visibility": "party",
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: RagContext(query="", chunks=[], context_block="", citations=[], skipped_reason="qdrant_down"),
    )
    monkeypatch.setattr(adapter_module, "LLMAdapter", FakeAdapter)

    req = ExtractionRequest(campaign_id=str(uuid.uuid4()), session_id=str(uuid.uuid4()))
    events, skips = extractor._llm_events(req, [_row(text="I promise to bury the bones.")])
