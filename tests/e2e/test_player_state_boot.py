...
def test_game_refetches_player_state_on_socket_reconnect(page: Page):
    player_state_calls: list[str] = []
    page.on(
        "request",
        lambda request: player_state_calls.append(request.url)
    ...
    page.evaluate(
        """() => {
            App.socket.emit('join_campaign', {
                campaign_id: App.campaign.id,
                session_id: App.sessionId,
                principal_id: App.principalId,
                join_token: App.localJoinTokenFor(App.authenticatedMember()),
            });
            App.loadPlayerStateSnapshot();
        }"""
    )
    page.wait_for_function(
        f"""() => performance.getEntriesByType('resource')
...
def test_control_handoff_event_triggers_player_state_refetch(page: Page):
    player_state_calls: list[str] = []
    page.on(
        "request",
        lambda request: player_state_calls.append(request.url)
    ...
    page.evaluate(
        """() => {
            const nextSeq = (App.lastEventSequence ?? 0) + 1;
            App.addEvent({
                event_id: 'control-handoff-e2e',
                seq_id: nextSeq,
                event_type: 'entity_control_changed',
                payload: {
                    entity_id: App.controlledEntities[0],
                    controller_principal_id: App.principalId,
                    control_version: 99,
                },
            });
        }"""
...
def test_two_tabs_same_principal_keep_same_visible_identity(page: Page, browser):
    other = browser.new_page()
    try:
        page.goto(f"{BASE_URL}/game?principal_id={PLAYER_A_ID}")
        other.goto(f"{BASE_URL}/game?principal_id={PLAYER_A_ID}")
        page.wait_for_function(
            "App?.playerState?.principal?.role === 'PLAYER'", timeout=10000
        )
        ...
        first = page.evaluate("() => App.playerState.principal.principal_id")
        second = other.evaluate("() => App.playerState.principal.principal_id")
        assert first == second == PLAYER_A_ID
        expect(page.locator("#principalSelect")).to_be_hidden()
        expect(other.locator("#principalSelect")).to_be_hidden()
    finally:
        other.close()
