...
- CHECK scope/principal coherence
Indexes:
...
2. Filter by caller principal visibility (`principal_id = ANY(visible_to)`).
...
- Policy: `current_setting('app.principal_id', true)::uuid = ANY(visible_to)`.

Fallback for non-RLS environments:
- Mandatory repository-layer filter on all reads using the same membership condition.
