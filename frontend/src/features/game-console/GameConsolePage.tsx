...
export default function GameConsolePage() {
  ...
  const resource = useAsyncResource(() => loadGameState(activeWorldId), [activeWorldId]);
  ...
  const selected = game?.entities.find((entity) => entity.entity_id === selectedEntityId) ?? game?.entities[0] ?? null;
  ...
  async function submitCommand(event: React.FormEvent) {
    ...
    setCommandError("");
    try {
      const result = await proposeCommand(activeWorldId, text, game, selected?.entity_id);
  ...
  return (
    <div className="page game-page">
      ...
      <PageHeader
        title="Game Console"
        description="Observe canonical reality, select an embodied actor, and submit typed proposals."
      ...
      <div className="game-grid">
        <Panel title="Harborport district" meta={<span>Turn {game.turn || "—"}</span>} className="map-panel">
          <div className="map-stage" aria-label="Tactical map of Harborport">
            <img src={MAP_URL} alt="Top-down map of Harborport docks and market" />
            {game.entities.slice(0, 4).map((entity) => (
              <button
                className={`map-token ${entity.entity_id === selected?.entity_id ? "selected" : ""}`}
                key={entity.entity_id}
                style={entityMapPosition(entity)}
                type="button"
                aria-label={`Select ${entity.name} on map`}
                onClick={() => setSelectedEntityId(entity.entity_id)}
              >{entity.name.slice(0, 1)}</button>
            ))}
          </div>
        </Panel>
        <Panel title="Entity rail" meta={<span>{game.entities.length} present</span>} className="entity-panel">
          <div className="selection-list compact" role="listbox" aria-label="Entities">
            {game.entities.map((entity) => <EntityRow key={entity.entity_id} entity={entity} selected={entity.entity_id === selected?.entity_id} onSelect={() => setSelectedEntityId(entity.entity_id)} />)}
          </div>
        ...
        <Panel title="Event feed" meta={<span>{events.length} visible events</span>} className="event-panel">
        ...
        <Panel title="Command composer" meta={<span>Proposal only</span>} className="command-panel">
          <form className="command-form" onSubmit={submitCommand}>
            <label>Acting entity<select value={selected?.entity_id ?? ""} onChange={(event) => setSelectedEntityId(event.target.value)}>{game.entities.map((entity) => <option key={entity.entity_id} value={entity.entity_id}>{entity.name}</option>)}</select></label>
            ...
            <button className="button primary" type="submit" disabled={sending || !command.trim()}><Send size={15} />{sending ? "Proposing…" : "Send proposal"}</button>
          </form>
...
function EntityRow({ entity, selected, onSelect }: { entity: EntitySummary; selected: boolean; onSelect: () => void }) {
  ...
  return (
    <button className={`selection-row ${selected ? "selected" : ""}`} type="button" role="option" aria-selected={selected} onClick={onSelect}>
      <span className="entity-sigil">{entity.name.slice(0, 1)}</span><span><strong>{entity.name}</strong><small>{entity.entity_type} · {entity.location ?? "Unknown"}</small><i className="health-track"><b style={{ width: `${health}%` }} /></i></span>
    </button>
  );
}
